  ...
  if (error) {
    ...
    await Promise.all(
      data.map(async (bundleCatalogue) => {
        ...
        bundleCatalogue.image = `https://thekettlegourmets3.s3.ap-southeast-1.amazonaws.com/bundleCatalogueImages/${replaced}-img`;
...
const getBundleCatalogue = async (req, res) => {
  try {
    ...
    // }
    // bundleCatalogue.image = productImg;
    bundleCatalogue.image = `https://thekettlegourmets3.s3.ap-southeast-1.amazonaws.com/bundleCatalogueImages/${bundleCatalogue.bundle.name}-img`;
