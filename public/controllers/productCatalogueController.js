  ...
  if (error) {
    ...
    await Promise.all(
      data.map(async (prodCatalogue) => {
        ...
        prodCatalogue.image = `https://thekettlegourmets3.s3.ap-southeast-1.amazonaws.com/productCatalogueImages/${replaced}-img`;
      })
    );
...
const getProductCatalogue = async (req, res) => {
  try {
    ...
    prodCatalogue.image = `https://thekettlegourmets3.s3.ap-southeast-1.amazonaws.com/productCatalogueImages/${prodCatalogue.product.sku}-img`;
